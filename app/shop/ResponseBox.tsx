...
type navigationProp = NativeStackNavigationProp<ParamList, 'ResponseBox'>;
type ResponseBoxRouteProp = RouteProp<ParamList, 'ResponseBox'>;
...
export default function ResponseBox() {
    ...
    const route = useRoute<ResponseBoxRouteProp>();
    ...
    const fetchMaKH = async () => {
      ...
      if(error){
        ...
        setMaKH(data.user_id);
        fetchNameUser(data.user_id);
      }
    }

    const fetchNameUser = async (user_id: number) => {
      const {data, error} = await supabase
      .from('Khachhang')
      .select('name')
      .eq('MaKH', user_id) 
          ...
          {messages.map((msg) => (
            <View
              ...
              style={[
                ...
                msg.sender_role === 'shop' ? styles.shopMessage : styles.userMessage,
              ]}
            >
